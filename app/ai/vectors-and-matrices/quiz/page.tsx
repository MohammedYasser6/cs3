...
export default function VectorsAndMatricesQuiz() {
  ...
  const moduleId = "vectors-and-matrices";
  const isAlreadyCompleted = completedModules?.includes(moduleId);

  const question =
    "In a neural network, what mathematical operation physically rotates, scales, or shears the input data vectors in order to find patterns and categorize information?";
  const options = [
    "Scalar Addition",
  ...
  const correctAnswerIndex = 1; // "Matrix Multiplication"

  const handleSubmit = () => {
    setIsSubmitted(true);

    if (selectedAnswer === correctAnswerIndex && !isAlreadyCompleted) {
      completeModule(moduleId, 150, "ai");
            ...
            <div className="flex items-center justify-between border-t border-slate-800 pt-6">
              {isSubmitted && selectedAnswer === correctAnswerIndex && (
                <span className="flex items-center gap-2 font-bold text-emerald-400 animate-pulse">
                  <CheckCircle className="h-5 w-5" /> +150 AI XP Earned!
                  Redirecting...
                </span>
              )}
              {isSubmitted && selectedAnswer !== correctAnswerIndex && (
