...
export default function ProfilePage() {
  ...
  const { xp, csXp, aiXp, cyberXp, level, completedModules } = useStore();
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="rounded-xl border border-cyan-900/30 bg-slate-900/50 p-6">
            ...
            <p className="text-sm text-slate-400">
              ...
              {completedModules?.filter(
                (m) => !m.startsWith("ai-") && !m.startsWith("cyber-"),
              ).length || 0}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-purple-900/30 bg-slate-900/50 p-6">
            ...
            <p className="text-sm text-slate-400">
              ...
              {completedModules?.filter(
                (m) =>
                  m.startsWith("what-is-ai") ||
                  m.startsWith("vectors-and-matrices"),
              ).length || 0}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-emerald-900/30 bg-slate-900/50 p-6">
            ...
            <p className="text-sm text-slate-400">Modules Completed: 0</p>
          </div>
        </div>
      </div>
    </div>
